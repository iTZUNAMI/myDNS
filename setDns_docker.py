...
if __name__ == "__main__":
    ...
    while True:
            print ("\n[-] myDNS \n")
            if (not myconfig_docker.bearer or not myconfig_docker.x_auth_email or not myconfig_docker.x_auth_key or not myconfig_docker.subdomain or not myconfig_docker.zone_id or not myconfig_docker.dns_record_id ):
                print("Configuration Error : missing some configuration, check myconfig_docker.py")
                ...
                if last_public_ip == None :
                    ...
                    updateDNS(current_public_ip,myconfig_docker.zone_id,myconfig_docker.dns_record_id,myconfig_docker.x_auth_email,myconfig_docker.x_auth_key)
                ...
                # wait x min for next exec, default 600 = 10 min
                now = datetime.now()
                # dd/mm/YY H:M:S
                comment = now.strftime("%d/%m/%Y %H:%M:%S")
                print ("Check at "+comment)
                time.sleep(int(myconfig_docker.time_repeat))  
